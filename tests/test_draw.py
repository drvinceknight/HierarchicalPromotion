...
def test_draw_included_edges():
    tikz_code = hrcy.state_to_tikz(capacities=capacities, state=state, include_edges=True)
    assert r'\draw' in tikz_code

def test_draw_include_boiler_plate():
    tikz_code = hrcy.state_to_tikz(capacities=capacities, state=state, include_boiler_plate=True)
